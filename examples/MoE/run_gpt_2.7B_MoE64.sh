...
HIDDEN_SIZE=2560
NUM_ATTN_HEADS=16
RAMPUP_BATCH_SIZE="256 256 13107200" # 13107200 samples = 50K iter * 256
...
DP_PARALLEL_SIZE=$(( ${NUM_GPUS} / ${PP_SIZE} ))
if [[ $EP_SIZE -gt $DP_PARALLEL_SIZE ]]; then
    EP_PARALLEL_SIZE=$DP_PARALLEL_SIZE
else
...
# 
# INIT_STD = sqrt(s / n), where s is a scale hyper-parameter and n is the number of input units
# in the weight tensor. The default Transformer initialization scale s = 1.0, while switch transformer
# (see https://arxiv.org/abs/2101.03961) suggests to set s to 0.1 in MoE model.
# 
# In summary, INIT_STD = sqrt(0.1 / 2560)
INIT_STD=0.00625
# INIT_STD=0.01
...
host="${HOSTNAME}"
NAME="gpt-${MODEL_SIZE}B-bs-${GLOBAL_BATCH_SIZE}-gpus-${NUM_GPUS}-mp-${MP_SIZE}-pp-${PP_SIZE}"
...
if [ "${USE_INTERNAL_DATA}" = "true" ]; then
    ...
    VOCAB_PATH=/cephfs/dzy/gpt/lambada/gpt2-vocab.json
    MERGE_PATH=/cephfs/dzy/gpt/lambada/gpt2-merges.txt
    # Public the Pile dataset, can be downloaded at https://mystic.the-eye.eu/public/AI/pile_neox/
    DATA_BLEND=$(ls /cephfs/dzy/new_nfs_data/pile_train/*.bin | awk '{print "1.0\t" substr($1, 1, length($1)-4)}')
fi
###############################################################################
data_options=" \
...
run_cmd="deepspeed ${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} | tee -a ${OUTPUT_BASEPATH}/log/${NAME}_${host}_${current_time}.log"
echo ${run_cmd}
eval ${run_cmd}
set +x
