...
> ℹ️ **Virtual pipeline parallelism (VPP).** A VPP `torch_dist` source sections the model per virtual chunk, so its keys are `model0.<param>`, `model1.<param>`, ... rather than a single bare-parameter namespace, and the `fsdp_dtensor` loader requests them as `model0.module.<param>`, `model1.module.<param>`. The converter preserves that section index: `model{i}.<param>` becomes `model{i}.module.<param>`, while single-section sources are unchanged. The loader-side preprocessing (`preprocess_fsdp_dtensor_state_dict`) likewise pairs each model section with its own chunk, and the save path now accepts the model *chunk list* rather than only the first chunk. **These VPP changes are not GPU-verified yet** — the 8×H100 verification described above covered `TP1/PP1/EP8` without virtual pipeline parallelism.
