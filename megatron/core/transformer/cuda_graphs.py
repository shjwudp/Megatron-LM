...
class _CudagraphGlobalRecord:
    ...
    def create_cudagraphs(cls):
        ...
        global USE_TQDM
        if USE_TQDM:
            # Print progress bar on only first and last ranks to reduce log clutter but still
            # give some sense of spread.
            if 0 < torch.distributed.get_rank() < (torch.distributed.get_world_size() - 1):
                USE_TQDM = False
            else:
        ...
        for g_idx, g in progress_bar:
            ...
            if USE_TQDM:
                ...
                if not (0 < torch.distributed.get_rank() < (torch.distributed.get_world_size() - 1)):
...
class _CudaGraphRunner(torch.nn.Module):
    ...
    def create_fwd_graph(self, args, kwargs, outputs=None, clone_inputs=True):
        ...
        def get_fwd_input_buffer(ten):
            ...
            elif hasattr(ten, "is_cudagraph_output") and hasattr(ten, "fwd_cudagraph_buffer"):
    ...
    def create_bwd_graph(self):
        ...
        for idx, o in enumerate(self.get_tensors(self.outputs)):
            out_grad = None
            if o.requires_grad:
                # See [interaction with recompute] in create_fwd_graph
                if hasattr(o, "is_cudagraph_input") and hasattr(o, "bwd_cudagraph_buffer"):
                    assert o.bwd_cudagraph_buffer.shape == o.shape
