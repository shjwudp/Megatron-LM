...
class FsdpParameterGroup:
    ...
    def unshard_parameters(self, orientation: str = "rowwise") -> None:
        ...
        if self._trace_pool_allocator is None:
            ...
            # Parameters are persistent objects. Rebind their data views after
            # the trace-pool assigns this logical key its physical slot.
            for index, fsdp_parameter in enumerate(self.fsdp_parameters):
                fsdp_parameter.unsharded.data = buffer.get_local_tensor(index)
    ...
    def reduce_partial_gradients(
        ...
        if self.main_grad.placements != self._accumulation_placements:
            if not has_sharded_grads:
                self._main_grad = self._allocate_main_grad(self._accumulation_placements)
            else:
                ...
                if changed_axis is None:
                    raise RuntimeError(
                        "FSDP gradient accumulation requires a changed placement axis."
                    )
                ...
                self._main_grad = restored_grad.redistribute(self._accumulation_placements)
