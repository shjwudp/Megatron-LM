...
class FsdpParameterGroup:
    ...
    def __init__(
        ...
        if fused_wgrad_is_complete and any(
            getattr(parameter, "zero_out_wgrad", False) for parameter in parameters
        ):
    ...
    def take_fused_wgrad_buffer(self) -> DBuffer:
        ...
        if self.fused_wgrad_is_complete:
            missing_fqns = [
                ...
                for fsdp_parameter in self.fsdp_parameters
                if not getattr(
                    fsdp_parameter.unsharded, "grad_added_to_main_grad", False
                )
            ]
            if missing_fqns:
                raise RuntimeError(
        ...
        for fsdp_parameter in self.fsdp_parameters:
            ...
            parameter.main_grad = None
            if self.fused_wgrad_is_complete:
                # TE returned a dummy autograd gradient only to run hooks. Every
                # real contribution is already in partial_grad for this contract,
                # so clean up here and avoid a second parameter-packing pass.
                parameter.grad = None
                parameter.grad_added_to_main_grad = False
        return partial_grad
