...
"""MXFP8 block quantization helpers for the minimal Megatron-FSDP path.

The production path delegates quantization of master weights to Transformer
Engine's verified ``cast_master_weights_to_fp8`` (see
``te_cast_master_weights_to_fp8`` and ``allocate_quantize_temp``); the
``set_*_payload`` / ``clear_payloads`` helpers rebind and detach the raw
fp8 payloads on the module's own MXFP8Tensor primary weights via TE's
``fp8_set_raw_data``.

This mirrors the reference MFSDP v2 MXFP8 support (PR #6197) but is grafted
minimally onto the static-order prefetch path: we only need the payload
binding helpers and the TE delegation shims, not the trace-pool allocator.
"""
