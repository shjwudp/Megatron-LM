...
**Experimental API requirement:** No-op.  The experimental API always stores raw
tensors backed by `DBuffer` on the module.  Sub-module parameters are always
accessible.  Provide a no-op method:
...
| `_replace_param_with_raw_if_needed()` | `() -> None` | No-op (params always raw in experimental API). |
...
- Add `_replace_param_with_raw_if_needed()` as no-op
...
| Line 74 | `fsdp_wrapper._replace_param_with_raw_if_needed()` | Method (no-op for experimental) |
