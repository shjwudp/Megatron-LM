...
represented by a shard view in this case, because an outer optimizer shard was
updated even though there is no separate tensor payload to copy. The next
unshard then refreshes the outer replicas. Quantized parameters require a
separate high-precision main-weight buffer.
