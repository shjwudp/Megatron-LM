...
storage for its direct meta parameters and buffers, runs its parameter reset
hook, and moves only those direct tensors to the target device. Non-recursive
application prevents a parent from trying to copy still-meta descendants, while
leaf-first reset ordering also lets parent reset hooks safely inspect or derive
state from initialized children.
