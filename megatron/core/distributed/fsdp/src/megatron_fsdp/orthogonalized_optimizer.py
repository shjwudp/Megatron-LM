...
PORT-NOTE (LANES-MUON): faithful port of the prototype's
`experimental/orthogonalized_optimizer.py` (`FsdpOrthogonalizedOptimizer`/`FsdpMuon`) onto current
main. Deviations from the prototype, each marked at its site below: (1) per lane file ownership this
module lives at `megatron_fsdp/orthogonalized_optimizer.py` instead of
`megatron_fsdp/experimental/`, so its `megatron_fsdp.experimental` sibling imports are qualified;
(2) RISK-3 API drift: `DBuffer.get_local_tensor` is spelled `get_tensor_view` on current main;
(3) `post_optimizer_rowwise`/`post_optimizer_colwise` (dev's `Fp8ParameterGroup` compute-weight
views) are consumed optionally via `getattr`, so parameter groups that do not expose them (main's
plain `FsdpParameterGroup`) skip that half of the local-shard consistency check. The shard planner
(`experimental.shard_plan`) keeps the prototype's `chunks=` owner balancing through
`experimental.owner_planning.assign_owner_work`."""
...
# PORT-NOTE (LANES-MUON): dev imported these as `.dbuffer`/`.parameter_group`/`.placement`/
# `.shard_plan` from its own `experimental/` directory; this module sits one level up (lane file
# ownership), so the `experimental` package is named explicitly. `shard_plan` is the ported
# prototype planner; `owner_planning` (its delegate) is upstream's planning library.
from .experimental.dbuffer import DBuffer
from .experimental.parameter_group import FsdpParameterGroup, get_containing_parameter_group
from .experimental.placement import Flat
from .experimental.shard_plan import (
    OwnerGatherPlan,
    OwnerScatterPlan,
    ShardPlan,
    assign_owner_work,
...
def _get_parameter_dp_group(param: DTensor) -> dist.ProcessGroup:
    ...
    if mesh.ndim == 2 and flat_axes == [0, 1]:
        # PORT-NOTE (LANES-MUON -> LANES-PLUMB): `mesh._mfsdp_flattened_group` is recorded by
        # the adapter's `_build_hybrid_dp_mesh(..., flattened_group=...)` (dev:mcore_fsdp_adapter
        # :1001-1053, called with `flattened_group=pg_collection.dp_cp` / `.expt_dp`). When it is
        # absent we fall back to recomputing the flattened group, matching dev's behavior.
...
def _compute_weight_local_views(parameter_group: FsdpParameterGroup) -> list[tuple[str, DBuffer]]:
    ...
    # PORT-NOTE (LANES-MUON -> LANES-CORE): `post_optimizer_rowwise`/`post_optimizer_colwise`
    # are the prototype's `Fp8ParameterGroup` compute-weight views (dev:parameter_group.py),
...
def _require_matching_local_shards(parameter_group: FsdpParameterGroup) -> None:
    ...
    for index, fsdp_parameter in enumerate(parameter_group.fsdp_parameters):
        ...
        for name, buffer in views:
            if buffer is None:
                continue
            # PORT-NOTE (LANES-MUON, RISK 3): dev's `DBuffer.get_local_tensor(index)` is spelled
            # `get_tensor_view(index)` on current main (DBuffer and QuantizedDBuffer alike).
            local_shape = buffer.get_tensor_view(index).shape
...
class FsdpOrthogonalizedOptimizer(torch.optim.Optimizer):
    ...
    def step(self, closure: Callable[[], float] | None = None) -> float | None:
        ...
        #
        # PORT-NOTE (LANES-MUON, PRESERVED KNOWN BEHAVIOR): with MXFP8 primary weights this
        # post-step sync is the prototype's "`--optimizer muon` + MXFP8 can hang in the
        # post-step quantization path" limitation (PR #6197 body). On dev the hang sat in
        # `_quantize_model_weight_from_main_weight` (MXFP8-only) inside this sync; on main the
        # same slot is `FsdpParameterGroup.sync_model_weight_from_main_weight()` ->
        # `post_optimizer_model_weight.quantize_(self.main_weight)`. Combined with the Adam
        # fallback bucket's `_step` -> fused-adam graph, rank 0 can end up blocked in the
        # post-step model-parallel all-reduce while its PP peer is still mid-step. This is a
        # faithful prototype rebuild: behavior preserved as-is, deliberately NOT fixed here.
