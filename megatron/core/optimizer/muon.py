...
def get_megatron_muon_optimizer(
    ...
    for model_chunk in model_chunks:
        ...
        # When TP > num_query_groups, the QKV weight is sharded across TP in a way that
        # mixes Q/K/V within each partition (see attention.py lines 1178-1196).
        # The activations go through AllGather before splitting, but the weight itself
        # cannot be cleanly split into Q/K/V components per partition.
        # In this case, we skip qkv splitting for muon.
        can_split_qkv = num_query_groups >= tp_size

        if can_split_qkv:
            # Normal case: weight is cleanly partitioned across TP
            # qkv_split_shapes is per query group (for the reshape/split in orthogonalize)
            qkv_split_shapes = [
            ...
            log_single_rank(
                logger,
                logging.WARNING,
                f'TP size ({tp_size}) > num_query_groups ({num_query_groups}). '
                f'Skipping qkv splitting for muon optimizer.',
            )
        ...
        for name, param in model_chunk.named_parameters():
            ...
            if 'linear_qkv.weight' in name and len(param.shape) == 2 and can_split_qkv:
                # validate that param shape is compatible with qkv_split_shapes
                expected_split_sum = sum(qkv_split_shapes)
                if param.shape[0] % expected_split_sum != 0:
                    raise ValueError(
                        f"QKV parameter '{name}' shape {param.shape} is incompatible with "
                        f"qkv_split_shapes {qkv_split_shapes}. First dimension {param.shape[0]} "
                        f"must be divisible by sum of qkv_split_shapes ({expected_split_sum})."
                    )
                # validate that num_query_groups would be positive
                computed_num_query_groups = param.shape[0] // expected_split_sum
                if computed_num_query_groups <= 0:
                    raise ValueError(
                        f"QKV parameter '{name}' shape {param.shape} would result in "
                        f"num_query_groups={computed_num_query_groups}. First dimension "
                        f"{param.shape[0]} must be >= sum of qkv_split_shapes ({expected_split_sum})."
                    )
                param.is_qkv = True
