...
``FsdpModule`` detects and routes to ``QuantizedDBuffer``-backed parameter
groups on the v2 side
...
class TestMegatronFSDPE2EMxfp8:
        ...
        for chunk_index, model_chunk in enumerate(model):
            ...
            for module_name, module in model_chunk.named_modules():
                for group in getattr(module, "_parameter_groups", []):
                    if not isinstance(group.model_weight, QuantizedDBuffer):
                        continue
    ...
    def test_parity_with_mfsdp_v1(self, case):
        ...
        # V2 owns explicit QuantizedDBuffer-backed parameter groups; require that
        # coverage and use their names to exclude representation-incompatible
        # parameter snapshots.
