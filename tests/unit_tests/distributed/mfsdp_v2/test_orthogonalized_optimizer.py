...
"""Owner-compute Muon: the sharded ``FsdpMuon`` step matches a single-rank Muon.

Step numerics run under `torchrun` and compare against a single-rank reference
using the same Newton-Schulz kernel (bitwise) and against `torch.optim.Muon`
(tolerance, since the kernels normalize differently); grouping, owner P2P and
the import guard pin the surrounding contracts.
"""
...
_EO_MOD = "megatron.core.distributed.fsdp.src.megatron_fsdp.orthogonalized_optimizer"
...
def test_import_guard_emerging_optimizers_available():
    ...
    from megatron.core.distributed.fsdp.src.megatron_fsdp import orthogonalized_optimizer as mod
...
def test_import_guard_without_emerging_optimizers():
    """Without emerging_optimizers the module degrades to placeholders and refuses construction."""
