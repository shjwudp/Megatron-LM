...
def test_sharded_grad_dtensor_reuse_across_microbatches(distributed_setup):
    ...
    # Every parameter in this toy model is owned by one of the FSDP groups above.
    sharded_params = list(model.parameters())
    assert sharded_params, "expected sharded parameters in the model"

    def train(model, optimizer) -> tuple[list[torch.Tensor], list[list[list[int]]]]:
        losses = []
        grad_identities: list[list[list[int]]] = []
        for step in range(3):
