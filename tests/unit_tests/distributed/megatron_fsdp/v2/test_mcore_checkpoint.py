...
class TestMegatronFsdpV2Checkpoint:
    ...
    def test_checkpoint_online_convert(
        ...
        *source_configs*, save via ``save_checkpoint``, load into an
        ...
        if source_type == "v2":
            src_sharding = source_configs.get("data_parallel_sharding_strategy", "optim_grads_params")
            tgt_sharding = target_configs.get("data_parallel_sharding_strategy", "optim_grads_params")
            if src_sharding != "optim_grads_params" or tgt_sharding != "optim_grads_params":
                pytest.skip("Only 'optim_grads_params' sharding strategy is supported for v2 source")
